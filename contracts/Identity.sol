...
contract Identity is ERC725 {
    ...
    bytes32 constant KEY_TOKEN = 0x544f4b454e000000000000000000000000000000000000000000000000000000;
    bytes32 constant KEY_LOGIN = 0x4c4f47494e000000000000000000000000000000000000000000000000000000;
    ...
    modifier onlyOwner() {
        require(toBytes32(msg.sender) == store[KEY_OWNER], "erc725-only-owner-allowed");
        _;
    }
    ...
    function transfer(address _token, address _to, uint256 _value) external {
        require(toBytes32(msg.sender) == store[KEY_TOKEN], "erc725-only-token-allowed");
